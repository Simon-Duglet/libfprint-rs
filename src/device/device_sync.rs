...
use gio::Cancellable;
use glib::translate::{FromGlibPtrContainer, FromGlibPtrNone};
...
impl FpDevice {
    ...
    pub fn list_prints_sync(&self, cancellable: Option<&Cancellable>) -> Result<Vec<FpPrint>, crate::GError> {
            ...
            let prints = unsafe { libfprint_sys::fp_device_list_prints_sync(self.to_glib_none().0, raw_cancel.cast(), std::ptr::addr_of_mut!(raw_error)) };

            if prints.is_null() {
                return Err(unsafe { glib::Error::from_glib_full(raw_error.cast()) });
            }
            
            let prints = prints.cast::<glib::ffi::GPtrArray>();
            let prints:Vec<FpPrint> = unsafe { FromGlibPtrContainer::from_glib_none(prints) };
